...
function complete(g) {
  var result = g.next();
  if (result.value === undefined || result.value.next === undefined) {
    return result.value;
  } else {
...
describe("interpreter", function() {
  ...
  describe("literals", function() {
    ...
    it("should be able to produce a string", function() {
      expect(complete(r('"hi my name is lauren"'))).toEqual("hi my name is lauren");
    });

    it("should be able to produce a string", function() {
      expect(complete(r("true"))).toEqual(true);
    });
  });

  // describe("pausing execution", function() {
  //   it("should be able to pause before execution of a lambda", function() {
  //     r("({1} ({2}))", undefined, function(k) {
  //       k(function(k) {
  //         k(function(k) {
  //           expect(k).toEqual(1);
  //         });
  ...
  describe("fn invocation", function() {
    ...
    describe("lambdas", function() {
      it("should be able to instantiate and invoke an empty lambda", function() {
        expect(complete(r(""))).toBeUndefined();
      });
      ...
      it("should be able to invoke a lambda on several args used in the lambda", function() {
        expect(complete(r('({?a ?b (add a b)} 1 2)'))).toEqual(3);
      });
    });
  });
